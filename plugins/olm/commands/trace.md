...
example: "/olm:trace kubernetes-nmstate-operator 4.16.3 --commit abc1234"
---
...
## Synopsis
```
/olm:trace <operator-name> <ocp-version> [--commit <sha>] [--bug <id>]
```

## Description
The `olm:trace` command traces an FBC (File-Based Catalog) operator release back to its source git revision. This is essential for answering the question: "Is my bug fix included in the shipped z-stream release?"
...
1. **Trace an operator release to its git revision:**
   ```
   /olm:trace kubernetes-nmstate-operator 4.16.3
   ```
